...
const nSquares = 10 * (config.difficulty + 1);
const squareHeight = canvasElement.height / nSquares;
const squareWidth = canvasElement.width / nSquares;
...
class Square {
  ...
  getMineNum() {
    ...
    for (; i < maxY; i++) {
      ...
      for (; j < maxX; j++) {
        ...
        if (!square || square.mine === false) continue;
        count += 1;
      }
    }
    return count;
  ...
  draw() {
    ...
    if (this.state & Square.STATE_OPEN) {
      if (this.state & Square.STATE_FLAG) this.state &= ~Square.STATE_FLAG;
      if (this.mine) {
        canvas.fillStyle = "#f03030";
        canvas.fillRect(this.posX, this.posY, this.width, this.height);
        canvas.drawImage(imgMine,
          this.posX + (this.width - imgMine.width) / 2,
          this.posY + (this.height - imgMine.height) / 2);
      }
      else {
        canvas.fillStyle = "#cccccc";
        canvas.fillRect(this.posX, this.posY, this.width, this.height);
        ...
        if (num === 0) {
          ...
          switch (num) {
            case 1: canvas.fillStyle = "#0000ff"; break;
            case 2: canvas.fillStyle = "#22ff22"; break;
            case 3: canvas.fillStyle = "#ff0000"; break;
            case 4: canvas.fillStyle = "#000088"; break;
            case 5: canvas.fillStyle = "#880000"; break;
            case 6: canvas.fillStyle = "#008800"; break;
            case 7: canvas.fillStyle = "#880088"; break;
            case 8: canvas.fillStyle = "#000000"; break;
          }
          canvas.font = squareHeight - 0.1 * squareHeight + "px Arial";
    ...
    if (this.state & Square.STATE_FLAG) {
      canvas.drawImage(imgFlag,
        this.posX + (this.width - imgFlag.width) / 2,
        this.posY + (this.height - imgFlag.height) / 2
      );
    }
  }
}
...
function init() {
  ...
  openSquares = 0;
  let row = 0, col = 0;
  for (; row < nSquares; row++) {
    col = 0;
    for (; col < nSquares; col++) {
      if (!squares[row]) squares.push([]);
...
function canvasMouseLeave(e) {
  ...
  if (prevSquare.state & Square.STATE_OPEN) return;
...
function canvasMouseUp(e) {
  ...
  square.state &= ~Square.STATE_CLICKED;
  square.draw();
  if (openSquares >= squareCount - mineSquares.length) end(false);
}
...
window.addEventListener('DOMContentLoaded', e => {
  ...
  imgMine.src = 'assets/mine.png';

  imgFlag.addEventListener('load', () => {
    flagLoaded = true;
    if (mineLoaded) {
      init();
    }
  })
  imgMine.addEventListener('load', () => {
    mineLoaded = true;
    if (flagLoaded) {
      init();
    }
